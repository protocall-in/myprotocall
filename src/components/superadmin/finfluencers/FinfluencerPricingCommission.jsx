...
import { FinInfluencer, User, PlatformSetting, RevenueTransaction } from '@/api/entities';
...
const PayoutTracking = ({ globalMinPayout, revenueData, finfluencers }) => {
    const [isLoading, setIsLoading] = useState(true);
    const [stats, setStats] = useState({ grossRevenue: 0, platformCommission: 0, netPayout: 0 });

    useEffect(() => {
        const grossRevenue = revenueData.reduce((sum, item) => sum + item.gross_amount, 0);
        const platformCommission = revenueData.reduce((sum, item) => sum + item.platform_commission, 0);
        const netPayout = revenueData.reduce((sum, item) => sum + item.influencer_payout, 0);
        setStats({ grossRevenue, platformCommission, netPayout });
        setIsLoading(false);
    }, [revenueData]);

    const handleExport = () => {
        const headers = ['Transaction Date', 'Finfluencer Name', 'Gross Amount', 'Platform Commission', 'Net Payout', 'Status'];
        const rows = revenueData.map(item => {
            const finfluencerName = finfluencers.find(f => f.id === item.influencer_id)?.display_name || 'Unknown';
            return [
                new Date(item.created_date).toLocaleDateString(),
                `"${finfluencerName}"`,
                item.gross_amount,
                item.platform_commission,
                item.influencer_payout,
                item.payout_status
            ].join(',');
        });
        ...
        link.setAttribute("download", "finfluencer_payout_tracking.csv");
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Card className="shadow-lg border-0 bg-white hover:shadow-xl transition-all duration-300 overflow-hidden">
                    <CardContent className="p-0">
                        <div className="bg-gradient-to-r from-purple-500 to-violet-600 p-4">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-white/90 text-sm font-medium">Net Payout to Finfluencers</p>
                        ...
                        <div className="p-4 bg-white">
                            <p className="text-xs text-gray-600">Total finfluencer earnings</p>
                        </div>
                    </CardContent>
                </Card>
            </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b">
                                ...
                                <th className="text-left py-2">Finfluencer</th>
                        ...
                        <tbody>
                            {revenueData.slice(0, 10).map(item => (
                                <tr key={item.id} className="border-b">
                                    <td className="py-2">{new Date(item.created_date).toLocaleDateString()}</td>
                                    <td>{finfluencers.find(f => f.id === item.influencer_id)?.display_name || 'N/A'}</td>
                                    <td className="text-right">₹{item.gross_amount.toFixed(2)}</td>
                                    <td className="text-right text-red-600">₹{item.platform_commission.toFixed(2)}</td>
                                    <td className="text-right text-green-600">₹{item.influencer_payout.toFixed(2)}</td>
...
const CommissionSettingsComponent = ({ platformSettings, moduleCommissionRate, onModuleRateChange, finfluencers, onIndividualRateChange }) => {
    return (
        <div className="space-y-6">
            <Card className="bg-blue-50 border-blue-200">
                ...
                <CardContent className="text-blue-800">
                    ...
                    <p className="text-sm mt-2">Override these settings below for the Finfluencers module.</p>
                </CardContent>
            </Card>

            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2"><DollarSign className="w-5 h-5 text-purple-600" />Finfluencer Module Commission Override</CardTitle>
                    <p className="text-sm text-gray-500">Override global commission rate for all finfluencers. Leave blank to use global rate ({platformSettings.global_commission_rate || '20'}%).</p>
                </CardHeader>
                <CardContent>
                    <div className="flex items-end gap-3 max-w-sm">
                        <div className="flex-grow">
                            <Label htmlFor="module-commission" className="flex items-center gap-1 mb-1"><Percent className="w-4 h-4 text-gray-500" /> Finfluencer Commission Rate (%)</Label>
                            <Input
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2"><Users className="w-5 h-5 text-green-600" />Individual Finfluencer Overrides</CardTitle>
                    <p className="text-sm text-gray-500">Set specific rates for individual finfluencers. These override both global and module settings.</p>
                </CardHeader>
                <CardContent className="space-y-4">
                    {finfluencers.map(finfluencer => (
                        <div key={finfluencer.id} className="flex items-center justify-between gap-4 border-b pb-3 last:border-0 last:pb-0">
                            <div>
                                <p className="font-semibold">{finfluencer.user?.display_name || finfluencer.display_name}</p>
                                <p className="text-xs text-gray-500">Current: {finfluencer.commission_override_rate || moduleCommissionRate || platformSettings.global_commission_rate || '20'}%</p>
                            </div>
                            <div className="flex items-center gap-2 w-48">
                                <Input
                                    type="number"
                                    placeholder="Override %"
                                    value={finfluencer.commission_override_rate || ''}
                                    onChange={(e) => onIndividualRateChange(finfluencer.id, e.target.value)}
                                />
                            </div>
                        </div>
                    ))}
...
export default function FinfluencerPricingCommission() {
    ...
    const loadData = useCallback(async () => {
        setIsLoading(true);
        try {
            const [settingsData, finfluencersData, revenueItems, usersData] = await Promise.all([
                PlatformSetting.list(),
                FinInfluencer.filter({ status: 'approved' }),
            ...
            setModuleCommissionRate(settingsMap.finfluencer_module_commission_rate || '');
            
            const usersMap = new Map(usersData.map(u => [u.id, u]));
            const populatedFinfluencers = finfluencersData.map(finf => ({
                ...finf,
                user: usersMap.get(finf.user_id),
            }));

            setFinfluencers(populatedFinfluencers);
            setRevenueData(revenueItems.filter(rt => finfluencersData.some(f => f.id === rt.influencer_id)));
    ...
    const handleSaveSettings = async () => {
        try {
            ...
            const moduleRateSetting = await PlatformSetting.filter({ setting_key: 'finfluencer_module_commission_rate' });
            const payload = { setting_key: 'finfluencer_module_commission_rate', setting_value: String(moduleCommissionRate) };
            ...
            const updatePromises = finfluencers.map(finf => {
                const rate = finf.commission_override_rate === '' ? null : parseFloat(finf.commission_override_rate);
                return FinInfluencer.update(finf.id, { commission_override_rate: rate });
            });
    ...
    const handleIndividualRateChange = (finfluencerId, value) => {
        setFinfluencers(prev => prev.map(finf => finf.id === finfluencerId ? { ...finf, commission_override_rate: value } : finf));
    };
